...
We rely on an ensemble-based semi-supervised learning process for defining an hybrid, i.e., ensemble, of truth
finding algorithms for information extraction systems. Let denote by $\qset$ the set of successive user queries processed 
by the information extraction system. For each query $\query$ in $\qset$ we have the corresponding set of claims $\claimset_{\query}$
returned by the extractor. We refer to the entire set of all claims by $\claimset$ regarding $\qset$.
