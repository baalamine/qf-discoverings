...
As a consequence and similarly to~\cite{Lu15}, we rely on an iterative active ensembling process which considers the set of claims $\claimset{}$, a set of testing claims
$\claimset_{GT}$, an adaptation set which is used for adapting the configuration of the ensemble, an initial ensemble size, a vector where each element
...
chooses exactly one set of claims about the same query for asking their labels to the user. Once these labels are acquired from the user, the algorithm  adds
the claims along with their truth labels into the training set. \lamine{Une esquisse du process d'apprentissage active}
