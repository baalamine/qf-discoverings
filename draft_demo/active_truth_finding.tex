...
In this study, we have used query by committee strategy which is proven to be very effective in many different settings.
...
extraction system by $\qset$. For each query $\query$ in $\qset$ we have the corresponding set of claims $\claimset_{\query}$ returned
by the extractor. We refer to the entire set of all claims by $\claimset$ regarding $\qset$.
