...
are moslty specific to a single channel/application/language and cannot be considered as ``agile''.
Moreover, each step of textual content analysis (from preprocessing, entity matching to classification) produces
systematic and random errors that need to be considered in truth discovery computation  in a deeper manner than the
very few attempts like \cite{GoasdoueKKLMZ13}). Tracing and estimating  the errors of information extraction, formatting, 
and linking  is one of the main challenges for automating truth discovery. 
